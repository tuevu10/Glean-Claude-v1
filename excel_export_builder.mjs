...
for (let i = 0; i < contractRows.length; i++) {
  ...
  f[18] = `=J${r}<>"NOT STARTED"`;
  f[19] = `=IF(O${r}=0,"",M${r}/Q${r})`;
  ...
  f[25] = `=IF(X${r}>0,Y${r}/X${r},0)`;
  f[26] = `=IF(Y${r}>0,V${r}/Y${r}-1,IF(V${r}=0,0,""))`;
  f[27] = `=AND(J${r}="ACTIVE",O${r}>='Assumptions'!B12,U${r}=30,X${r}=30,V${r}-Y${r}>='Assumptions'!B11,OR(AND(Y${r}>0,AA${r}>'Assumptions'!B10),AND(Y${r}=0,V${r}>0)))`;
  ...
  f[39] = `=IF(M${r}>='Assumptions'!B13,"OVER ENTITLEMENT",IF(J${r}="NOT STARTED","NOT STARTED",IF(AND(J${r}="ACTIVE",OR(AE${r}>'Assumptions'!B6,AND(AG${r}<>"",AG${r}<D${r}-'Assumptions'!B7))),"EARLY EXHAUSTION RISK",IF(AND(Q${r}>='Assumptions'!B9,AE${r}<'Assumptions'!B8),"UNDERUTILIZING","ON TRACK"))))`;
  f[40] = `=IF(AN${r}="OVER ENTITLEMENT","P1 · Urgent",IF(AN${r}="EARLY EXHAUSTION RISK","P2 · High",IF(OR(AN${r}="UNDERUTILIZING",AND(AN${r}="ON TRACK",OR(J${r}="EXPIRED",AB${r}))),"P3 · Medium","P4 · Routine")))`;
  f[41] = `=IF(AN${r}="OVER ENTITLEMENT","Revenue Accounting / Billing",IF(AN${r}="EARLY EXHAUSTION RISK","Sales / Account Management",IF(AN${r}="UNDERUTILIZING","Customer Success",IF(OR(J${r}="EXPIRED",AB${r}),"Sales / Account Management","No Action"))))`;
  const baseAction = `IF(AN${r}="OVER ENTITLEMENT","Reconcile credit ledger and verify overage terms; coordinate a top-up or contract amendment.",IF(AN${r}="NOT STARTED","Wait for contract activation; no usage forecast is available.",IF(AN${r}="EARLY EXHAUSTION RISK","Review forecast with the account team and discuss additional credits before exhaustion.",IF(AN${r}="UNDERUTILIZING","Review adoption barriers, agree a usage plan, and assess renewal implications.","Continue routine monitoring."))))`;
  ...
  f[43] = `=IF(AN${r}="OVER ENTITLEMENT","Consumed credits have reached or exceeded entitlement.",IF(AN${r}="NOT STARTED","The contract has not started; forecast-dependent values are unavailable.",IF(AN${r}="EARLY EXHAUSTION RISK",IF(AE${r}>'Assumptions'!B6,"Projected utilization exceeds the configured threshold. ","")&IF(AND(AG${r}<>"",AG${r}<D${r}-'Assumptions'!B7),"Estimated exhaustion is earlier than the configured contract-end buffer.",""),IF(AN${r}="UNDERUTILIZING","Enough contract time has elapsed and projected utilization is below the configured threshold.","No primary exception rule is triggered."))))`;
  f[44] = `=IF(AND(U${r}>0,U${r}<30),"Short trailing window: rate uses eligible contract days",IF(AND(W${r}=0,K${r}<E${r}),"No recent rate; exhaustion date unavailable",IF(J${r}="NOT STARTED","Forecast unavailable before contract start","")))`;
